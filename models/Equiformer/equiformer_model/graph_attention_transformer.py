...
@register_model
def graph_attention_transformer_l2(num_basis=128,
    atomref=None, task_mean=None, task_std=None, **kwargs):
    model = GraphAttentionTransformer(
        irreps_node_embedding='32x0e+16x1e+8x2e',  # Reduced from 128/64/32
        num_layers=4,  # Reduced from 6
        irreps_node_attr='1x0e',
        irreps_sh='1x0e+1x1e+1x2e',
        fc_neurons=[32, 32],  # Reduced from [64, 64]
        irreps_feature='256x0e',  # Reduced from 512
        irreps_head='16x0e+8x1e+4x2e',  # Reduced from 32/16/8
        num_heads=2,  # Reduced from 4
        irreps_pre_attn=None,
        rescale_degree=False,
        nonlinear_message=False,
        irreps_mlp_mid='192x0e+96x1e+48x2e',  # Reduced from 384/192/96
        norm_layer='layer',
        alpha_drop=0.1,  # Reduced from 0.2
        proj_drop=0.0,
        out_drop=0.0,
        drop_path_rate=0.0,
        mean=task_mean,
...
# Assume necessary O3 modules and functions (o3.Irreps, LinearRS, Activation, etc.) are imported
# --- Gated Fusion Module ---
...
class GraphAttentionTransformerMulti(nn.Module):
    def __init__(self,
                 irreps_in='5x0e',
                 irreps_node_embedding='128x0e+64x1e+32x2e', num_layers=6,
                 irreps_node_attr='1x0e', irreps_sh='1x0e+1x1e+1x2e',
                 max_radius=5.0,
                 number_of_basis=128, basis_type='gaussian', fc_neurons=[64, 64],
                 irreps_feature='256x0e',  # Reduced feature dimension
                 irreps_head='32x0e+16x1o+8x2e', num_heads=4, irreps_pre_attn=None,
                 rescale_degree=False, nonlinear_message=False,
                 irreps_mlp_mid='128x0e+64x1e+32x2e',
                 norm_layer='layer',
                 ...
                 drop_path_rate=0.0,
                 mean=None, std=None, scale=None, atomref=None,
                 clip_encoder: int = 32,  # Final CLIP embedding dimension (after clip_head)
                 clip_input_dim: int = 768,  # Input CLIP embedding dimension (e.g., 768)
                 global_feature_dim: int = 256  # Graph-level feature dimension (set by head output)
                 ):
        super().__init__()
        ...
        self.irreps_feature = o3.Irreps(irreps_feature)
        self.num_layers = num_layers
        self.irreps_edge_attr = o3.Irreps(irreps_sh) if irreps_sh is not None else o3.Irreps.spherical_harmonics(
            self.lmax)
        self.fc_neurons = [self.number_of_basis] + fc_neurons
        self.irreps_head = o3.Irreps(irreps_head)
        self.num_heads = num_heads
        ...
        if self.out_drop != 0.0:
            self.out_dropout = EquivariantDropout(self.irreps_feature, self.out_drop)
        # --- Modified head: output a vector of dimension global_feature_dim ---
        self.head = nn.Sequential(
            LinearRS(self.irreps_feature, self.irreps_feature, rescale=_RESCALE),
            Activation(self.irreps_feature, acts=[nn.SiLU()]),
            LinearRS(self.irreps_feature, o3.Irreps(f'{global_feature_dim}x0e'), rescale=_RESCALE)
        )
        self.scale_scatter = ScaledScatter(_AVG_NUM_NODES)

        # --- New: Add clip branch and gated fusion ---
        self.clip_input_dim = clip_input_dim
        self.clip_encoder = clip_encoder
        # Process clip embeddings: map from clip_input_dim to 256, then to clip_encoder.
        self.clip_head = nn.Sequential(
            nn.Linear(self.clip_input_dim, 256),
            nn.ReLU(),
            nn.Linear(256, self.clip_encoder)
        )
        # Gated fusion: fuse graph-level features (global_feature_dim) with clip features.
        self.gated_fusion = GatedFusion(mol_dim=global_feature_dim, clip_dim=self.clip_encoder,
                                        fusion_dim=global_feature_dim)
        # --- End new section ---

        # Final linear layer: map fused features to a scalar prediction.
        self.final_lin = nn.Linear(global_feature_dim, 1)
    ...
    def no_weight_decay(self):
        ...
        for module_name, module in self.named_modules():
            if (isinstance(module, nn.Linear)
                    ...
                    or isinstance(module, GaussianRadialBasisLayer)):
                for parameter_name, _ in module.named_parameters():
                    if isinstance(module, nn.Linear) and 'weight' in parameter_name:
                        continue
                    global_parameter_name = module_name + '.' + parameter_name
                    assert global_parameter_name in named_parameters_list
                    no_wd_list.append(global_parameter_name)
    ...
    def forward(self, f_in, pos, batch, node_atom, clip_embeddings=None, **kwargs) -> torch.Tensor:
        # Compute graph edges and spherical harmonics.
        ...
        # Global aggregation: obtain graph-level features.
        global_features = self.scale_scatter(self.head(node_features), batch, dim=0)  # [B, global_feature_dim]
        ...
        if clip_embeddings is not None:
            ...
            global_features = self.gated_fusion(global_features, clip_feat)
        # --- End new fusion ---

        outputs = self.final_lin(global_features)
        if self.scale is not None:
            outputs = self.scale * outputs
...
def graph_attention_transformer_l2_multi(atomref=None, task_mean=None, task_std=None,
                                   **kwargs):
    model = GraphAttentionTransformerMulti(
        irreps_node_embedding='32x0e+16x1e+8x2e',  # Reduced from 128/64/32
        num_layers=4,  # Reduced from 6
        irreps_node_attr='1x0e',
        irreps_sh='1x0e+1x1e+1x2e',
        ...
        scale=None,
        atomref=atomref,
        clip_encoder=32,  # New: final clip embedding dimension
        clip_input_dim=768,  # New: input clip embedding dimension
        global_feature_dim=256  # New: graph-level feature dimension (matching head output)
    )
    return model